...
function FormInput<T extends FieldValues>({ register, type = "text", name, label, errors, isTextarea, ...rest }: Props<T>) {
    ...
    return (
        <div className="relative z-0">
            {isTextarea ? <textarea {...register} id={name} className={twMerge(inputClassName, "min-h-[200px] max-h-[400px]")} placeholder="" {...(rest as DetailedHTMLProps<TextareaHTMLAttributes<HTMLTextAreaElement>, HTMLTextAreaElement>)} /> : <input {...register} type={type} id={name} {...rest} className={inputClassName} placeholder="" />}
